...
class nested:
    def __init__(self, loglike, priorTransform, nlive, ndims, maxiter=100000,
                 outputname="outputs/test", mc_scale=1.0):
    ...
    def sampling(self, dlogz=0.01):
        ...
        # p_i -> posterior weights like in Skilling's paper in Section 13
    ...
    def mcmc_explore(self, uworst, logLstar, cov, naccepted=100):
        ploglike = logLstar
        accepted = 0
        rejected = 0

        while accepted < naccepted:
            while True:
                # Generate a candidate point
                tryu = np.random.multivariate_normal(uworst, cov)
